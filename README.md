...
| `REDMINE_EASY_ENABLED` | No | `false` | Enable Easy Redmine support: issue reads return `easy_sprint` (`{id, name, due_date}`), `easy_sprint_phase`, `easy_sprint_position` and `easy_story_points`; `update_redmine_issue` accepts `easy_sprint_id`, `easy_story_points` and `target_backlog`; `list_redmine_issues` accepts an `easy_sprint_id` filter; and `list_easy_sprints`, `list_easy_attendances`, `manage_easy_attendance`, `delete_easy_attendance` and `approve_easy_attendances` are registered. Easy Redmine is a fork rather than a plugin, so nothing extra has to be installed -- the flag only says the server is talking to one ([details](docs/tool-reference.md#easy-redmine-sprints)) |
| `REDMINE_EASY_DB_URL` | No | - | Read-only MySQL DSN (`mysql://user:password@host:3306/easyredmine`), used only by `list_easy_sprints`. Easy Redmine serves no sprint endpoint, so a sprint *name* cannot be resolved over HTTP. Setting a sprint by id needs neither this nor the driver. Requires the `easy` extra (`pip install 'redmine-mcp-server[easy]'`) |
